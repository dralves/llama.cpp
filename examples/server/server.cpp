...
// ADD openssl headers for SHA-256 (assuming your build links libssl / libcrypto):
...
// REPLACE the simple_hash function with a new SHA-256 function:
...
int main(int argc, char ** argv) {
    ...
    // Create the server_context object, but no HTTP setup
    ...
    // Instead of a single prompt, we read each line in "my_prompt.txt"
    // and generate a response for it. Now we also open an output file.
    std::ifstream file("my_prompt.txt");
    if (!file.is_open()) {
        fprintf(stderr, "Failed to open file \"my_prompt.txt\".\n");
        return 1;
    }

    // Open a file to append all prompts/responses
    std::ofstream out("my_conversation.txt", std::ios::app);
    if (!out.is_open()) {
        fprintf(stderr, "Failed to open \"my_conversation.txt\" for writing.\n");
        return 1;
    }

    std::string line;
    while (std::getline(file, line)) {
        // Build a single server_task for each line read
        server_task task(SERVER_TASK_TYPE_COMPLETION);
        task.id    = 0;
        task.index = 0;

        // Use default parameters (no JSON)
        json empty_data;
        ...
        // Tokenize the line
        auto prompts = tokenize_input_prompts(ctx_server.vocab, line, /* add_special= */ true, /* do_lower_case= */ true);
        if (prompts.empty()) {
            continue; // skip empty lines
        }
        task.prompt_tokens = prompts[0];

        // LAUNCH SLOT - DO NOT EXIT ON FAILURE:
        server_slot * slot = ctx_server.get_available_slot(task);
        if (!slot) {
            fprintf(stderr, "No available slot, skipping.\n");
            // previously: return 1; -> replaced with continue
            continue; // skip to next line
        }
        if (!ctx_server.launch_slot_with_task(*slot, task)) {
            fprintf(stderr, "Failed to launch task for line, skipping.\n");
            // previously: return 1; -> replaced with continue
            continue;
        }

        // Print "Prompt: ..." in the new format
        std::cout << "Prompt: " << line << "\n";
        out        << "Prompt: " << line << "\n";

        // We'll capture the incremental generation
        size_t oldSize = 0;
        while (slot->is_processing() && slot->has_next_token) {
            ctx_server.update_slots();
            if (slot->generated_text.size() > oldSize) {
                std::string chunk = slot->generated_text.substr(oldSize);
                oldSize = slot->generated_text.size();

                // print chunk to terminal
        ...
        // NEW: label the final text "Response: "
        std::cout << "\n\n" << "Response: " << slot->generated_text << "\n";
        out        << "\n\n" << "Response: " << slot->generated_text << "\n";

        // Compute SHA-256
        std::string hash_value = compute_sha256(slot->generated_text);

        // Print and write the new "Hash: ..."
